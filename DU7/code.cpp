...
{
  public:
    CIndex ( const T tmp, const C & cmpp = C() ) 
    : text(tmp),
      cmp(cmpp)
    {}
    set<size_t> search ( const T & key ) {

      set<size_t> indexes;

      // finds if these are equal from Jan Matousek
      auto is_equal = [&](const auto & lhs, const auto & rhs) {
          // cout << "Comparing " << lhs << " vs " << rhs << endl;
      ...
      for ( auto it1 = text.begin(); it1 != text.end(); it1 ++ ) {
        auto it = it1;
        for ( auto it2 = key.begin(); it2 != key.end(); it2 ++ ) {
          if( !is_equal( *it, *it2) ) {
            found = false;
            break;
          }
